...
import { CameraShape, getCameraDimensionsForWidth, normalizeCameraShape, traceCameraShapePath } from '../shared/cameraShapes';
...
/** Compute even canvas dimensions (<= 4K) from a source video element. */
...
export function drawWebcam(
    ...
    const safeInset = 16;
    const normalizedShape = normalizeCameraShape(wc.shape);
    ...
    const x = Math.max(safeInset, Math.min(canvas.width - targetWidth - safeInset, centerX - targetWidth / 2));
    const y = Math.max(safeInset, Math.min(canvas.height - targetHeight - safeInset, centerY - targetHeight / 2));

    ctx.save();
    ctx.beginPath();
    traceCameraShapePath(ctx, normalizedShape, x, y, targetWidth, targetHeight);
    ctx.clip();
