...
`WINDOWS_STORE_RESERVED_NAME` must exactly match a name reserved for the product in Partner Center, because Forge writes it into the MSIX manifest `DisplayName`.
...
- In Partner Center, leave only `Windows.Desktop` selected unless you are intentionally shipping packages for other device families. Do not enable Xbox unless you also provide an Xbox-compatible neutral or `x64` package.
